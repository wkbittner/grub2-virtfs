 ...
 *  
 *  Adapted to GRUB by: Natalie Orlin <norlin@us.ibm.com>
 *
 *  Virtio code adapted from gpxe
 *
...
#define BUG() do { \
   grub_printf("BUG: failure at %s:%d/%s()!\n", \
                   ...
                   void *opaque, int num_added)
{
  ...
  for (i = head; out; i = vr->desc[i].next, out--) {
    ...
    vr->desc[i].addr = (grub_uint64_t) virt_to_phys((unsigned long)list->addr);
